...
# default: rsuntk/KernelSU
DEFAULT_KSU_REPO="https://raw.githubusercontent.com/rsuntk/KernelSU/main/kernel/setup.sh"
# end of default args
...
usage() {
	...
	printf "\tLLVM: Use all llvm toolchains to build: (atleast 4.19.148, opt: 1/0)\n"
	printf "\tLLVM_IAS: Use llvm integrated assembler: (atleast 4.19.148, opt: 1/0)\n"
	exit;
}
...
if [ -z $DEVICE ]; then
	echo -e "\$DEVICE is unassigned, use 'export DEVICE=<device_model>'"
	DEVICE="Unknown"
fi
if [[ "$KERNELSU" = "true" ]]; then
    curl -LSs $DEFAULT_KSU_REPO | bash -s main
else
